...
public class CustomInternalServerError extends Exception implements ExceptionMapper<InternalServerErrorException> {
    ...
    public static final String TEXT_INTERNAL_SERVER_ERROR = "Hi!  Sorry for the error, but it's a custom server error!";

    public CustomInternalServerError() {
        super(TEXT_INTERNAL_SERVER_ERROR);
    }

    public CustomInternalServerError(final String string) {
        super(string);
    }

    @Override
    public Response toResponse(final InternalServerErrorException exception) {
        return Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(TEXT_INTERNAL_SERVER_ERROR
                + "\t=>\t"
